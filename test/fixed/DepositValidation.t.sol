    ...
    function test_Deposit_validation_collateralTokenCap_borrowATokenCap() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                Errors.COLLATERAL_TOKEN_CAP_EXCEEDED.selector, size.fixedConfig().collateralTokenCap, amount * 1e18
            )
        );
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                Errors.BORROW_ATOKEN_CAP_EXCEEDED.selector, size.fixedConfig().borrowATokenCap, amount * 1e6
            )
        );
