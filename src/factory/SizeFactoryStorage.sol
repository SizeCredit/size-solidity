...
import {Nonce, NonceBitmap} from "@src/factory/libraries/NonceBitmapLibrary.sol";
...
abstract contract SizeFactoryStorage {
    ...
    // mapping of `nonce | bitmap` of authorized actions for operators per account (added on v1.7)
    mapping(address operator => mapping(address onBehalfOf => NonceBitmap nonceBitmap)) public authorizations;
    // mapping of authorization nonces per account (added on v1.7)
    mapping(address onBehalfOf => Nonce nonce) public authorizationNonces;
}
