...
const REPAYS_QUERY = `
  query GetRepays($where: Repay_filter) {
    repays(subgraphError: allow, where: $where) {
      id
      debtPositionId
      blockNumber
      blockTimestamp
...
let arraysDebtPositionId = [];

// Get price of ticker using binance api
...
async function fetchRepays() {
  const response = await axios.post(subgraphUrl, {
    query: REPAYS_QUERY,
    variables: { where: {} },
  });
  const repays = response.data.data.repays;

  // Sort repays, the last one going to the first on the new array
  const sortedRepays = repays.sort(
    (a, b) => a.blockTimestamp - b.blockTimestamp
  );

  arraysDebtPositionId = sortedRepays.map((repay) => repay.debtPositionId);

  return sortedRepays;
}

async function fetchAllPositionId() {
  const response = await axios.post(subgraphUrl, {
    query: CREATE_CREDIT_POSITION_QUERY,
    variables: { where: {} },
  });
  const creditPositions = response.data.data.createCreditPositions;

  arraysCreatePositionId = creditPositions.map(
    (position) => position.creditPositionId
  );

  return arraysCreatePositionId;
}

async function checkIfClaimed(currentDebtPositionId) {
  try {
    const response = await axios.post(subgraphUrl, {
      query: `
        query CheckClaim($creditPositionId: BigInt!) {
          claims(where: {creditPositionId: $creditPositionId}) {
            id
            blockNumber
            blockTimestamp
            transactionHash
      ...
      variables: {
        creditPositionId: currentDebtPositionId,
      },
    });

    const responseIfTrue = response.data.data.claims;
...
async function fetchCreateCreditPositions() {
  let matchedCreditPositions = [];

  // Get the last debtPositionId
  const lastDebtPositionId =
    arraysDebtPositionId[arraysDebtPositionId.length - 1];

  const response = await axios.post(subgraphUrl, {
    query: CREATE_CREDIT_POSITION_QUERY,
    variables: { where: { debtPositionId: lastDebtPositionId } },
  });
  const creditPositionsData = response.data.data.createCreditPositions;

  matchedCreditPositions.push(
    ...creditPositionsData.map((position) => ({
      creditPositionId: position.creditPositionId,
    }))
  );

  return matchedCreditPositions;
}

async function claimCreditPositions(matchedCreditPositions) {
  const contract = new ethers.Contract(sizeContractAddress, abi, signer);

  for (const position of matchedCreditPositions) {
    try {
      const creditPositionId = ethers.BigNumber.from(position.creditPositionId);
      const claimParams = { creditPositionId };

      const claimExecute = await contract.claim(claimParams);
      console.log(claimExecute);
    } catch (error) {
    /*  console.error(
        `Error claiming creditPositionId: ${position.creditPositionId}`,
        error
      ); */
    }
  }
...
async function claimCreditPositionsIfNotClaimedYet(position) {
  const contract = new ethers.Contract(sizeContractAddress, abi, signer);

  try {
    const creditPositionId = ethers.BigNumber.from(position);
    const claimParams = { creditPositionId };

    const claimExecute = await contract.claim(claimParams);
    console.log(claimExecute);
  } catch (error) {
    //console.error(`Error claiming creditPositionId: ${position}`, error);
  }
  //
}
...
try{
 ...
 exports.handler = async function (event, context) {
  ...
  try {
    const sortedRepays = await fetchRepays();

    if (sortedRepays.length >= 1) {
      const lastDebtPositionId =
        sortedRepays[sortedRepays.length - 1].debtPositionId;
      const matchedCreditPositions = await fetchCreateCreditPositions(
        lastDebtPositionId
      );
      console.log("Lastest PositionID Repayed:", lastDebtPositionId);
      try {
        await claimCreditPositions(matchedCreditPositions);
        console.log("Completed processing matchedCreditPositions", {
          matchedCreditPositions,
        });
      } catch (error) {
    ...
    console.log("now going to start the Liquidate check");
  ...
  // Claim All the Position if not claimed yet //
  try {
    const allPosition = await fetchAllPositionId();
    console.log(allPosition);

    for (let i = allPosition.length - 1; i >= 0; i--) {
      const currentCreditPositionId = allPosition[i];

      let check = await checkIfClaimed(currentCreditPositionId);

      if (check !== true) {
        console.log("is Claimed before?", check);
        await claimCreditPositionsIfNotClaimedYet(currentCreditPositionId);
        console.log(
          `Completed processing for debtPositionId: ${currentCreditPositionId}`
        );
      }
    }

    console.log("Completed processing Claim all Position available");
  } catch (error) {
    console.error("Error in Autotask:", error);
  }
}
